...
export default function Lists({ }) {
    ...
    useEffect(() => {
        ...
        apiFetch(`https://api.music.rockhosting.org/api/admin/lists?r=${limits.bottom}:${limits.top}&${Object.keys(search).map(key => `${key}=${search[key]}`).join('&')}`, session).then(data => data.json()).then(data => {
