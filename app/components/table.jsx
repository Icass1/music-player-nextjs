...
export function Table({ data, onSearch, limits, handleScroll }) {
    ...
    return (
        <>
            ...
            <div
                ref={scrollRef}
                className="overflow-scroll absolute top-8 left-0 right-2 bottom-2 rounded-lg"
                onClick={() => {
                    rowShowing == "" ? "" : setRowShowing("");
                }}
            >
                <table
                    className={clsx("h-fit transition-[opacity]", {
                        "opacity-25": rowShowing !== "",
                    })}
                >
                    <thead className="sticky top-0 ">
                        <tr>
                            ...
                            {data.columns.map((column) => (
                                <td
                                    key={column.name}
                                    className="bg-1 border-neutral-700 border border-solid min-w-52"
                                >
                                    <div className="m-1 flex flex-col">
                                        <label className="flex flex-row">
                                            <label className="ml-0 mr-auto">
                                                {column.name}
                                            </label>
                                            <label className="ml-auto mr-0">
                                                {column.type}
                                            </label>
                                        </label>
                                        <input
                                            className="bg-3 rounded-sm pl-1 pr2"
                                            placeholder={`Search by ${column.name}`}
                                            onInput={(e) => {
                                                search.current[column.name] =
                                                    e.target.value;
                    ...
                    <tbody>
                        {data.rows.map((row, index) => (
                            ...
                            >
                                <td className="bg-1 min-w-10 text-center border-neutral-700 border-r border-solid sticky left-0">
                                    {index + 1 + limits.bottom}
                                </td>
                                {data.columns.map((column) => (
                                    <td
                                        key={column.name}
                                        className={clsx(
                                            "bg-2 h-8 items-center",
                                            { "bg-3": index % 2 == 1 },
                                        )}
                                    >
...
function RowMenu({ originalRowShowing, data }) {
    ...
    useEffect(() => {
        ...
        if ("current_list" in originalRowShowing) {
            ...
            apiFetch(`/api/list/${originalRowShowing["current_list"]}`)
                .then((response) => response.json())
                .then((data) => {
                    setRowData((oldValue) => {
                        return { ...oldValue, current_list: data };
                    });
                });
        }
        if ("current_song" in originalRowShowing) {
            ...
            apiFetch(`/api/song/info/${originalRowShowing["current_song"]}`)
                .then((response) => response.json())
                .then((data) => {
                    setRowData((oldValue) => {
                        return { ...oldValue, current_song: data };
                    });
                });
        }
        ...
        if ("songs" in originalRowShowing) {
            JSON.parse(originalRowShowing.songs)
                .slice(0, 10)
                .map((song) => {
                    setRowData((oldValue) => {
                        return {
    ...
    return (
        <div className="bg-neutral-600 shadow-lg rounded-lg absolute left-10 right-20 top-20 bottom-20 z-10 text-white overflow-y-auto">
                ...
                ? Object.keys(rowShowing).map((key, index) => (
                      <div key={index} className="flex flex-col m-3 gap-1">
                          ...
                          <input
                              ...
                              onInput={(e) => {
                                  handleRawChange(e, key);
                              }}
                              value={rowShowing[key]}
                          ></input>
                      </div>
                  ))
                : Object.keys(rowShowing).map((key, index) => (
                                              ...
                                              <>
                                                  ...
                                                  <div className="flex flex-row gap-2 bg-neutral-700 rounded-md">
                                                      <Image
                                                          ...
                                                          src={`https://api.music.rockhosting.org/api/list/image/${rowShowing[key]}_300x300`}
                                                          height={50}
                                                          width={50}
                                                          alt=""
                                                      />
                                          ...
                                          return (
                                              <>
                                                  ...
                                                  <div className="flex flex-row gap-2 bg-neutral-700 rounded-md">
                                                      <Image
                                                          className="rounded-tl-md rounded-bl-md"
                                                          src={`https://api.music.rockhosting.org/api/song/image/${rowShowing[key]}_300x300`}
                                                          height={50}
                                                          width={50}
                                                          alt=""
                                                      />
                                              ...
                                              <>
                                                  ...
                                                  <div className="bg-neutral-700 grid grid-cols-2 gap-2 p-2 rounded-md">
                                                      {JSON.parse(
                                                          ...
                                                          <div
                                                              key={list}
                                                              className="bg-neutral-600 rounded-sm flex flex-row gap-1 w-full"
                                                          >
                                                              <Image
                                                                  alt=""
                                                          ...
                                                          .map((song) => (
                                                              ...
                                                              >
                                                                  <Image
                                                                      alt=""
                                                                      className="rounded-sm"
                                                                      src={`https://api.music.rockhosting.org/api/song/image/${song}_100x100`}
                                                                      width={50}
                                                                      height={
                                                                          50
                                                                      }
            ...
            <div className="-translate-x-1/2 left-1/2 relative w-fit flex flex-row gap-6 mb-3">
                <button className="text-red-300 hover:bg-red-600 hover:text-white rounded-lg p-1">
                    Delete
                </button>
                <button
                    className="bg-green-600 rounded-lg p-1 disabled:opacity-50"
