...
export default function Lists({}) {
    ...
    useEffect(() => {
        ...
        apiFetch(
            `https://api.music.rockhosting.org/api/admin/lists?r=${limits.bottom}:${limits.top}&${Object.keys(
                search,
            )
            ...
            .then((data) => {
                setData({
                    ...
                    total_showing: data.total_rows_showing,
                });
    ...
    const handleScroll = useCallback(
        (e) => {
            if (
                e.target.scrollTop ==
                e.target.scrollHeight - e.target.offsetHeight + 10
            ) {
                if (loading.current) {
                    return;
                }
                setLimits((oldLimits) => {
                    return { bottom: 0, top: oldLimits.top + 10 };
                });
            }
        },
        [loading],
        ...
        <>
            <div className="flex flex-row gap-7">
                ...
                <label className="text-orange-500">
                    When edit DB is available, change directory path of lists
                    that starts with AllSongs
                </label>
            </div>
            {(function () {
                switch (data) {
