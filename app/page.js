...
export default function Home() {
    ...
    useEffect(() => {
        ...
        if (mainRef.current) {
            ...
            mainRef.current.parentNode.onscroll = (e) => {
    ...
    return (
        <>
            <div className="flex flex-row md:hidden mt-3 ml-3 gap-4">
                <Link
                    className="relative block md:hidden fg-1 w-12 h-12 rounded-full"
                    href="/user"
                >
                    <Image
                        ...
                        src={
                            session.status == "authenticated"
                                ...
                                : "https://api.music.rockhosting.org/images/user.svg"
                        }
                        width={40}
                        height={40}
                        alt="User icon"
                ...
                if (session.status == "unauthenticated") {
                    return (
                        <div className="block relative top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96">
                            <label className="block relative text-xl text-center">
                                You are not loged in.
                            </label>
                            <Link
                                className="block relative font-bold text-2xl text-green-600 text-center"
                                href="/login"
                            >
                                Login
                            </Link>
                    ...
                    return (
                        <div className="relative text-3xl font-bold top-1/2 left-1/2 w-fit -translate-x-1/2 -translate-y-1/2">
                            Loading your lists...
                        </div>
                    );
                    ...
                    return (
                        <div className="relative w-fit top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                            <div className="w-auto text-center text-3xl font-bold">
                                It feels lonely here...
                            </div>
                            <div className="flex md:flex-row flex-col gap-2">
                                <div className="w-auto text-center text-xl font-bold ml-2 mr-2">
                                    Click in the search button to add lists to
                                    your library
                                </div>
                                <Link
                                    className="block relative left-1/2 -translate-x-1/2 md:left-0 md:-translate-x-0 w-fit"
                                    href="/search"
                                >
                                    <Image
                                        className="block invert-[0.8] hover:invert-[0.7] select-none"
...
function AddContextMenu({ children, item, setDownloadProgress, setMusicData }) {
    const { randomQueue, handlePlayList, setQueue, queue, queueIndex } =
    ...
    useEffect(() => {
        ...
        const eventSource = new EventSource(
            `https://api.music.rockhosting.org/api/compress-list/${downloadingID}`,
        );
    ...
    const handleRemoveList = (id) => {
        apiFetch(`/api/user/remove-list`, session, {
            method: "POST",
        ...
        <ContextMenu
            options={{
                ...
                "Add to bottom of queue": () =>
                    handleAddListToBottomQueue(item.id),
                "Remove from library": () => {
...
function GridLayout({ musicData, setMusicData }) {
    return (
        <div
            ...
            style={{
                gridTemplateColumns: `repeat(auto-fill, minmax(${innerWidth > 768 ? "200px" : "150px"}, 1fr))`,
            }}
        >
            {musicData.map((item) => (
                <GridContainer
        ...
        >
            ...
            >
                ...
                <div className="flex flex-col mt-3 mb-3">
                    <label
                        className={clsx(
                            "ml-3 mr-3 text-lg fade-out-neutral-200 font-bold cursor-pointer overflow-y-hidden min-w-0 max-w-full",
                            { "fade-out-fg-1": item.id == currentList },
                        )}
                    >
                        {item.name}
                    </label>
                    <label
                        className={clsx(
                            "ml-3 mr-3 text-base fade-out-neutral-200 cursor-pointer min-w-0 max-w-full",
                            { "fade-out-fg-2": item.id == currentList },
                        )}
                    >
                        {item.author}
                    </label>
    ...
    return (
        ...
        >
            ...
            >
                ...
                ></Image>
                <div
                    className="grid h-[50px]"
                    style={{ gridTemplateRows: "max-content max-content" }}
                >
                    <label
                        className={clsx(
                            "pl-3 pr-3 text-lg fade-out-neutral-200 font-bold cursor-pointer h-6 overflow-y-hidden min-w-0 max-w-full",
                            { "fade-out-fg-1": item.id == currentList },
                        )}
                    >
                        {item.name}
                    ...
                    <label
                        className={clsx(
                            "pl-3 pr-3 fade-out-neutral-200 cursor-pointer min-w-0 max-w-full",
                            { "fade-out-fg-2": item.id == currentList },
                        )}
                    >
                        {item.author}
        ...
        <>
            {Object.keys(listsByName).map((author) => (
                <div key={author} className="m-2 mb-4">
                    <label className="text-2xl md:text-4xl font-bold">
                        {author}
                    </label>
                    <div
                        className="grid gap-2 mt-1"
                        style={{
                            gridTemplateColumns: `repeat(auto-fill, minmax(${innerWidth > 768 ? "350px" : "150px"}, 1fr))`,
                        }}
                    >
                        {listsByName[author].map((item) => (
                            ...
                            >
                                <Link
                                    href={`/list/${item.id}`}
                                    className={clsx(
                                        "rounded-lg grid grid-cols-2 bg-3 md:hover:brightness-110 transition-all items-center shadow-lg h-12 md:h-[50px]",
                                    )}
                                    style={{
                                        gridTemplateColumns:
                                        ...
                                        background:
                                            downloadProgress.id == item.id
                                                ? `linear-gradient(90deg, rgb(100 100 100) 0%, rgb(100 100 100) ${downloadProgress.progress - 5}%, rgb(64 64 64) ${downloadProgress.progress}%, rgb(64 64 64) 100%)`
                                                : "",
                                    }}
                                >
                                    <Image
                                        ...
                                        width={50}
                                        height={50}
                                        className="rounded-lg w-12 h-12 md:w-[50px] md:h-[50px]"
                                        alt={item.name}
                                    ></Image>
                                    <label
                                        className={clsx(
                                            "ml-2 text-lg md:text-2xl pr-3 fade-out-neutral-200 font-bold cursor-pointer min-w-0 max-w-full",
                                            {
                                                "fade-out-fg-1":
                                                    item.id == currentList,
                                            },
                                    ...
                                    {item.id == currentList && isPlaying ? (
                                        <Equalizer
                                            className="w-8 md:w-20 h-full p-1"
                                            bar_count={
                                                innerWidth > 768 ? 15 : 6
                                            }
                                            bar_gap={1}
