...
export default function User() {
    ...
    useEffect(() => {
        ...
        apiFetch(`/api/user/get-last-played`, session)
            ...
            .then((data) => {
                ...
                for (let repeatedArtist of byArtist) {
                    ...
                    let song = data
                        .map((song) => {
                            if (song.artist == repeatedArtist[0]) {
                                return song;
                            }
                        })
                        .filter((title) => title)[0];
                    mostListenedArtists.push({
                        id: song.id,
                        artist: song.artist,
                        times_played: repeatedArtist[1],
                    });
                }
                ...
                for (let repeatedAlbum of byAlbum) {
                    ...
                    let song = data
                        .map((song) => {
                            if (song.album == repeatedAlbum[0]) {
                                return song;
                            }
                        })
                        .filter((title) => title)[0];
                    mostListenedAlbums.push({
                        id: song.id,
                        cover_url: song.cover_url,
                        artist: song.artist,
                        album: song.album,
                        times_played: repeatedAlbum[1],
                    });
                }
                ...
                for (let repeatedSong of bySong) {
                    ...
                    let song = data
                        .map((song) => {
                            if (song.id == repeatedSong[0]) {
                                return song;
                            }
                        })
                        .filter((title) => title)[0];

                    mostListenedSongs.push({
                        id: song.id,
                        cover_url: song.cover_url,
                        title: song.title,
                        artist: song.artist,
                        album: song.album,
                        times_played: repeatedSong[1],
                    });
                }
                ...
                for (let repeatedGenre of byGenre) {
                    ...
                    let song = data
                        .map((song) => {
                            if (song.genre == repeatedGenre[0]) {
                                return song;
                            }
                        })
                        .filter((title) => title)[0];
                    mostListenedGenres.push({
                        id: song.id,
                        genre: song.genre,
                        times_played: repeatedGenre[1],
                    });
                }
    ...
    if (session.status == "loading") {
        return (
            <label className="block relative font-bold text-2xl text-center top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                Loading...
            </label>
        );
    }
        ...
        <>
            <div className="relative block max-w-[1000px] left-1/2 -translate-x-1/2">
                ...
                {lastSongsPlayed == null || lastSongsPlayed == undefined ? (
                    <label className="ml-6 mt-3 text-xl font-bold text-neutral-300 relative top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                        Loading...
                    </label>
                    ...
                    <>
                        <div className="max-w-[400px] ml-auto mr-auto md:max-w-none">
                            <div className="flex flex-col">
                                ...
                                <div
                                    className="block md:grid md:ml-4 md:mr-4"
                                    style={{
                                ...
                                >
                                    ...
                                    {mostPlayedSong?.map((song) => (
                                        <div
                                            key={song.id}
                                            className="bg-neutral-800 w-10/12 ml-auto mr-auto mt-6 rounded-lg flex flex-col"
                                        >
                                            <Image
                                                src={song.cover_url.replace(
                                                    "_50x50",
                                                    "",
                                                )}
                                                width={0}
                                            ...
                                            <div className="mr-4 ml-4 mt-3 mb-3 flex flex-col">
                                                <label
                                                    className="text-xl font-bold fade-out-white"
                                                    title={song.title}
                                                >
                                                    {song.title}
                                                </label>
                                ...
                                <label className="block font-bold text-2xl mt-6 ml-4">
                                    Most listened album
                                </label>
                                <div
                                    className="block md:grid md:ml-4 md:mr-4"
                        ...
                        {innerWidth > 768 ? (
                            <ComputerView lastSongsPlayed={lastSongsPlayed} />
                        ) : (
                            <MobileView lastSongsPlayed={lastSongsPlayed} />
                        )}
                    </>
                ) : (
                    <></>
...
function MobileView({ lastSongsPlayed }) {
    return Object.keys(lastSongsPlayed).map((timeAgo, index) => (
        <div key={index}>
            <label className="block ml-4 mr-4 font-bold text-base text-neutral-400 text-left sticky top-0 bg-2">
                {timeAgo}
            </label>
            {lastSongsPlayed[timeAgo].map((song, index) => (
                <div
                    key={index}
                    className="ml-4 mr-4 mt-2 mb-2 flex gap-2 items-center"
                >
                    <Image
                        className="rounded"
                        src={song.cover_url}
                    ...
                    <div className=" min-w-0 w-full">
                        <div className="text-xl fade-out-neutral-200">
                            {song.title}
                        </div>
                        <div className=" text-lg fade-out-neutral-400">
                            {song.artist}
                        </div>
                    </div>
                </div>
