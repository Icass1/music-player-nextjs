        ...
        if (React.isValidElement(child)) {
            let newProps = {
                onContextMenu: (e) => {
                    ...
                    if (
                        e.clientX + contextMenuRef.current.offsetWidth >
                        window.innerWidth
                    ) {
                        x = e.clientX - contextMenuRef.current.offsetWidth;
                    }
                    ...
                    if (
                        e.clientY + contextMenuRef.current.offsetHeight >
                        window.innerHeight
                    ) {
                        y = e.clientY - contextMenuRef.current.offsetHeight;
                    }
    ...
    return (
        <>
            ...
            <div
                ref={contextMenuRef}
                className={clsx(
                    `fixed bg-[#252525e0] rounded-lg z-50 w-max border-solid border-neutral-700 border-1`,
                    { invisible: !showing },
                )}
            ...
            >
                {Object.keys(options).map((key) => (
                    <div
                        key={key}
                        className="p-2 rounded-lg md:text-sm text-lg hover:bg-neutral-600 cursor-pointer opacity-100"
                        onClick={() => {
                            options[key]();
                            setShowing(false);
                        }}
