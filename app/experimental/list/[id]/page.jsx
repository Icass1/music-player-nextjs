                ...
                <Table>
                    ...
                    <TableBody className="gap-10">
                        {filteredSongs.map((song, index) => {
                            ...
                            return (
                                <TableRow
                                    className={`border-b-0 h-12 cursor-pointer hover:bg-muted/50 ${isCurrentSong ? "bg-muted" : ""}`}
                                    key={song.id}
                                    onClick={() => {
                                        handlePlayList(musicData.id, song.id);
                                    }}
                                    variant={"ghost"}
                                >
                                    ...
                                    <TableCell className="py-2 max-w-1 truncate">
                                        <label className="font-medium truncate cursor-pointer text-primary">
                                            {song.title}
                                        </label>
                                    </TableCell>
                                ...
                                >
                                    <TableCell className="py-1 px-0 w-14 text-center cursor-pointer">
                                        <div className="w-14 h-14 relative">
                                            <Image
                                                className={`ml-auto mr-auto absolute ${isCurrentSong && isPlaying ? "opacity-30" : ""}`}
                                            ...
                                            {isCurrentSong && isPlaying ? (
                                                <Equalizer
                                                    bar_count={8}
                                                    bar_gap={2}
                                                    className="absolute left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 h-12 w-10"
                                                    centered={true}
                                                    toggleCenter={false}
                                                ></Equalizer>
                                            ) : (
                                    ...
                                    <TableCell className="py-2 max-w-1 truncate">
                                        <div className="flex flex-col">
                                            <label className="truncate cursor-pointer text-primary font-medium">
                                                {song.title}
                                            </label>
                                            <label className="truncate cursor-pointer text-sm text-muted-foreground">
                                                {song.artist}
                                            </label>
                                        </div>
                                    </TableCell>
                                    <TableCell className="py-2 max-w-1 truncate">
                                        <label className="py-2 truncate cursor-pointer ">
                                            {song.album}
                                        </label>
                                    </TableCell>
                                    <TableCell className="py-2 w-4">
...
function ListHeader({ musicData, setSearchTerm, searchTerm }) {
    ...
    const handleDownload = () => {
        ...
        const eventSource = new EventSource(
            `https://api.music.rockhosting.org/api/compress-list/${musicData.id}`,
        );
    ...
    return (
        <>
            ...
            <div className="p-6 flex items-end space-x-6">
                <Image
                    src={`https://api.music.rockhosting.org/api/list/image/${musicData.id}_300x300`}
            ...
            <div className="px-6 py-4 flex items-center space-x-4">
                {currentList == musicData.id && isPlaying ? (
                    ...
                    <Button
                        ...
                        className="rounded-full"
                        onClick={() => {
                            currentList == musicData.id
                                ? handlePlay()
                                : handlePlayList(musicData.id);
                        }}
                    >
