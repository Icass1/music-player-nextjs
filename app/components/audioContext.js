...
const AudioProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        if (
            audio.src ==
                `https://api.music.rockhosting.org/api/song/${currentSong.id}` ||
        ...
        if ("mediaSession" in navigator) {
            navigator.mediaSession.metadata = new MediaMetadata({
                ...
                artwork: [
                    {
                        src: `https://api.music.rockhosting.org/api/song/image/${currentSong.id}`,
                        sizes: "640x640",
                        type: "image/jpeg",
                    },
                ],
    ...
    useEffect(() => {
        ...
        apiFetch(`/api/song/${song.id}`, { signal: signal })
            .then((data) => data.blob())
            .then((blob) => {
                console.log("Cached");
                console.log(blob);
    ...
    useEffect(() => {
        ...
        audio.onended = function () {
            apiFetch(
                "https://api.music.rockhosting.org/api/user/end-song",
                session,
                {
                    method: "POST",
                    body: JSON.stringify({ song_id: currentSong }),
                },
            );
                ...
                apiFetch(`/api/list/${id}`)
                    ...
                    .then((musicData) => {
                        ...
                        let _songsList = [...musicData.songs].filter((song) =>
                            song.in_database == false || song.status != "ready"
                                ? false
                                : true,
                        );
                        console.log("_songsList", _songsList);

                        let song = _songsList.filter(
                            (tempSong) => tempSong.id == startSong,
                        )[0];
