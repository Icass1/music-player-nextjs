...
export function Song({ type, musicData, checkMusicData, index, song, listId }) {
    ...
    function handlePlayClick() {
        ...
        if (currentSong.id == song.id && currentList == listId) {
            ...
            let _songsList = musicData.songs.filter((song) => {
                if (song.in_database == false) {
                    return false;
                } else {
                    return true;
            ...
            let list = _songsList
                .slice(index + 1)
                .concat(_songsList.slice(0, index));
    ...
    const handleDownload = () => {
        ...
        downloadAndSaveMusic(
            `https://api.music.rockhosting.org/api/song/${song.id}`,
            song.id,
        );
        setStored(true);
    };
    ...
    const handleDownloadToDatabase = useCallback(() => {
        ...
        const id = song.spotify_url.replace(
            ...
            options={Object.assign(
                ...
                song.in_database == false
                    ? {
                          ...
                          "Copy Spotify URL": () => {
                              navigator.clipboard.writeText(song.spotify_url);
                          },
                          "Copy Spotify ID": () => {
                              navigator.clipboard.writeText(
                                  song.spotify_url.replace(
                                      "https://open.spotify.com/track/",
                                      "",
                                  ),
                              );
                    ...
                    : {
                          ...
                          Share: () => {
                              navigator.clipboard.writeText(
                                  `https://music.rockhosting.org/song/${song.id}`,
                              );
                          },
                      },
                queue
                    ...
                    .includes(song.id)
                    ? {
                          "Move song to next in queue":
                              handleMoveSongToNextInQueue,
                      }
                    : {},
            )}
        >
            <div
                ...
                className="relative ml-3 mr-3 mt-2 mb-2"
            >
                {type == "Album" ? (
                    <AlbumSong
                        key={index}
                        ...
                        handleDownloadToDatabase={handleDownloadToDatabase}
                    ></AlbumSong>
                ) : (
                    <PlaylistSong
                        key={index}
...
function AlbumSong({
    ...
    listId,
    downloadProgress,
    handleDownloadToDatabase,
}) {
    const { currentSong, currentList, isPlaying } =
        useContext(MediaPlayerContext);
    ...
    return (
        <div
            className={clsx(
                "grid items-center rounded-md h-9 md:h-12 gap-2 transition-colors",
                {
                    "md:hover:bg-neutral-800 cursor-pointer":
                        song.in_database !== false,
                },
            )}
            style={{
        ...
        >
            {song.id == currentSong.id && isPlaying && currentList == listId ? (
                <Equalizer
                    className="w-6 md:w-[50px] h-full top-0"
                    bar_count={innerWidth > 768 ? 10 : 5}
                    bar_gap={1}
                    centered={true}
                    toggleCenter={false}
                />
                ...
                <label
                    className={clsx("text-sm w-6 md:w-[50px] text-center ", {
                        "text-fg-1":
                            song.id == currentSong.id && currentList == listId,
                        "text-neutral-400": !(
                            song.id == currentSong.id && currentList == listId
                        ),
            ...
            <label
                className={clsx(
                    "relative text-lg md:text-2xl fade-out-neutral-300 min-w-0 max-w-full",
                    {
                        "fade-out-fg-1":
                            song.id == currentSong.id && currentList == listId,
                        "cursor-pointer": song.in_database !== false,
            ...
            {song.in_database == undefined ? (
                ...
                <SVG
                    ...
                    onClick={
                        !song.in_database ? handleDownloadToDatabase : () => {}
                    }
                />
            )}
            {stored ? <label>ok</label> : <label></label>}
            <label
                className={clsx("md:text-xl text-base w-[35px] md:w-[60px]", {
                    "text-fg-1":
                        song.id == currentSong.id && currentList == listId,
                    "text-neutral-400": !(
                        song.id == currentSong.id && currentList == listId
...
function PlaylistSong({
    ...
    const { currentSong, isPlaying, currentList } =
        useContext(MediaPlayerContext);
    ...
    song.cover_url = song.cover_url.replace(
        "_50x50",
        innerWidth > 768 ? "_50x50" : "_100x100",
    );

    return (
        <div
            className={clsx(
                "grid gap-x-2 rounded-md cursor-pointer items-center transition-colors",
                { "hover:bg-neutral-800": song.in_database !== false },
            )}
            style={{
                gridTemplateColumns:
                    innerWidth > 768
        ...
        >
            <div className="relative h-[50px]">
                ...
                listId == currentList ? (
                    <>
                        <Image
                            className="rounded-md absolute opacity-20"
                            alt={song.title}
                            src={song.cover_url}
                            width={50}
                            height={50}
            ...
            <div className="flex flex-col cursor-pointer min-w-0 max-w-full">
                <label
                    className={clsx(
                        "cursor-pointer md:text-xl text-base fade-out-neutral-200 min-w-0 max-w-full",
                        {
                            "fade-out-fg-1":
                                song.id == currentSong.id &&
                                listId == currentList,
                ...
                <label
                    className="min-w-0 max-w-full cursor-pointer grid items-center"
                    style={{
                        gridTemplateColumns: "max-content max-content 1fr",
                    }}
                >
                    {song.has_lyrics ? (
                        <label className="col-start-1 col-end-2 text-xs mr-1 bg-neutral-600 text-neutral-100 text-opacity-65 rounded-sm h-min pl-1 pr-1">
                            LYRICS
                        </label>
                    ) : (
                        <></>
                    ...
                    {song.has_dynamic_lyrics ? (
                        <label className="col-start-2 col-end-3 text-xs mr-1 bg-neutral-600 text-neutral-100 text-opacity-65 rounded-sm h-min pl-1 pr-1">
                            DYNAMIC LYRICS
                        </label>
                    ) : (
                        <></>
                    ...
                    <label
                        className={clsx(
                            "fade-out-neutral-300 md:text-base text-sm min-w-0 max-w-full col-start-3 col-end-4",
                            {
                                "fade-out-fg-2":
                                    song.id == currentSong.id &&
                                    listId == currentList,
            ...
            <label
                className={clsx(
                    "hidden md:block fade-out-neutral-200 min-w-0 max-w-full",
                    {
                        "fade-out-fg-2":
                            song.id == currentSong.id && listId == currentList,
                    },
                )}
                title={song.genre}
            >
            ...
            <Link
                ...
                className={clsx(
                    "hidden md:block fade-out-neutral-200 min-w-0 max-w-full hover:fade-out-neutral-400",
                    {
                        "fade-out-fg-2 hover:fade-out-fg-1":
                            song.id == currentSong.id && listId == currentList,
                    },
                )}
                title={song.album}
            ...
            <label
                className={clsx(
                    "fade-out-neutral-100 md:text-xl text-sm min-w-0 max-w-full text-center",
                    {
                        "fade-out-fg-1":
                            song.id == currentSong.id && listId == currentList,
                    },
